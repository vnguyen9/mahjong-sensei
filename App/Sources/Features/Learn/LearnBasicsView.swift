...
/// Lane 4 · Learn — "The tiles" primer. A friendly, bilingual walk through the
/// three suits, the honors, the bonus flowers/seasons, and how a winning hand
/// is built (4 sets + 1 pair). All examples are real `MahjongTileView`s.
struct LearnBasicsView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                MJBackHeader(title: "The tiles") { dismiss() }
                ScrollView {
    ...
    private func suitBlock(name: String, zh: String, jyut: String, note: String, tiles: [Tile]) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                TileRow(tiles, theme: .jade, width: 30, spacing: 5)
                    .padding(.vertical, 2)
            }
        }
    }
    ...
    private var bonusCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Flowers 梅蘭菊竹 and seasons 春夏秋冬. They're never part of a set — you set them aside and each one can add faan on its own.")
                ...
                .fixedSize(horizontal: false, vertical: true)
                .lineSpacing(2)
            TileRow([.flower(.plum), .flower(.orchid), .flower(.chrysanthemum), .flower(.bamboo)],
                    theme: .jade, width: 30, spacing: 5)
            TileRow([.season(.spring), .season(.summer), .season(.autumn), .season(.winter)],
                    theme: .jade, width: 30, spacing: 5)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .mjCard()
    ...
    private var buildingCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("A win is 14 tiles: four sets and one pair. A set is a chow, a pung, or a kong.")
                .font(MJFont.ui(11.5))
                .foregroundStyle(MJColor.cream(0.6))
                .fixedSize(horizontal: false, vertical: true)
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    meldChip([.p(2), .p(3), .p(4)], "Chow", "順子", "A run of three")
                    meldChip([.s(5), .s(5), .s(5)], "Pung", "刻子", "Three the same")
                    meldChip([.east, .east, .east, .east], "Kong", "槓", "Four the same")
                    meldChip([.whiteDragon, .whiteDragon], "Pair", "對子", "The “eyes”")
                }
                .padding(.vertical, 2)
            }
    ...
    private func meldChip(_ tiles: [Tile], _ label: String, _ zh: String, _ note: String) -> some View {
        VStack(spacing: 6) {
